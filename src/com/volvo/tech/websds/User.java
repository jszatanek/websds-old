...
public class User extends WebsdsApplication{
  ...
  public boolean getAllUsers(String pCompanyId) throws Exception {
    try{
      ...
      String wstrSql =  " select u.user_id "+
                        ...
                        "   ,u.lastname "+
                        "   ,r.NAME as role_name "+
                        "   ,DATE_FORMAT(u.LAST_LOGIN,'"+super.getDateTimeFormat()+"') as last_login "+
  ...
  public boolean getUser(String pUserId) throws Exception{
    try {
      String wstrSql =  " select u.user_id "+
                        ...
                        "   , DATE_FORMAT(last_login, '" + getDateTimeFormat() + "') as last_login "+
                        "   , is_user_removable_yn "+
                        "   , company "+
                        "   , r.role_id "+
                        "   , r.name as role_name"+
      ...
      if(super.getNextRow()){
        ...
        this.password = super.getCurrCol("pw");
        ...
        this.lastLogin = super.getCurrCol("last_login");
  ...
  boolean wbUserExists = false;

    try {
    /*
      if(pUser.equals("ADMIN") && pPassword.equals("ADMIN")){
        return true;
      } // w8 med anpassningar så att AD-lösenordet används.
      else 
      */
...
// check if user exists
   makeConnection();     
    ...
    wbUserExists = getUser(pUser.toUpperCase());   

   closeConnection();

    if (wbUserExists){
   
          if (pUser.toUpperCase().equalsIgnoreCase(userId.toUpperCase()) &&
              pPassword.equalsIgnoreCase(password)){
           return true;
          } else {return false;}     
    //      return true;
    } else { return false; }
